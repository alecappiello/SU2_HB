...
void CGeometry::UpdateGeometry(CGeometry **geometry_container, CConfig *config) {
  ...
  for (iMesh = 1; iMesh <= config->GetnMGLevels(); iMesh++) {
    ...
    //geometry_container[iMesh]->SetGridVelocity_HB(geometry_container[iMesh-1]);
...
void CGeometry::SetGridVelocity_HB(CGeometry ***geometry_container, CConfig *config_container, unsigned short setZone){
	unsigned short iZone, jDegree, iDim, iMGlevel;
		  ...
		  su2double period = config_container->GetHarmonicBalance_Period();//config_container[ZONE_0]->GetTimeSpectral_Period();
		  period /= config_container->GetTime_Ref();
		  su2double deltaT = period/(su2double)(config_container->GetnTimeInstances());
		  ...
		  for (iMGlevel = 0; iMGlevel <= config_container->GetnMGLevels(); iMGlevel++) {
		      ...
		      for (iDim = 0; iDim < nDim; iDim++) {
		        ...
		        /*--- Store grid velocity for this point, at this given dimension, across the Zones ---*/
		    	  for (iZone = 0; iZone < nZone; iZone++) {
		    		  ...
		    		  Coord = geometry_container[iZone][iMGlevel]->node[iPoint]->GetCoord();
		    		  if (reset)
		    			  geometry_container[setZone][iMGlevel]->node[iPoint]->SetGridVel(iDim, 0);//fitted_velocities[iZone]
		    		  else
		    			  geometry_container[setZone][iMGlevel]->node[iPoint]->SetGridVel(iDim, fitted_velocities[setZone]);
		    	  }
		      }
		    }
		  }
